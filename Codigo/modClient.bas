...
'If App.PrevInstance Or (FindWindow(vbNullString, Form_Caption) > 0) Then
...
If RawServersList = "" Or RawServersList = "<h1>Service Unavailable</h1>" Then
    ...
    RawServersList = "server.imperiumao.com.ar:7666:Argentina (Desconocido);secundario.imperiumao.com.ar:7666:Secundario (Desconocido);spain.imperiumao.com.ar:7666:España (Desconocido);localhost:7666:Local (Desconocido)"
Else
    ServersRecibidos = True
End If
...
frmPres.Picture = General_Load_Picture_From_Resource("presentacion")
...
Do While prgRun
    ...
    If RequestPosTimer > 0 Then
        ...
        If RequestPosTimer = 0 Then
            ...
            Call ClientTCP.Send_Data(Request_Pos)
        End If
    End If
    ...
    If EngineRun Then
        If frmMain.WindowState <> vbMinimized Then
            ...
            If (Audio = 1 Or Musica <> CONST_DESHABILITADA) Then Call Sound.Sound_Render
...
Audio = Val(Leer.DarValor("INIT", "SonidoHabilitado"))
...
Call General_Var_Write(Arch, "INIT", "Musica", str(Musica))
...
'6. Deshabilitamos los timers
Call BuffersBorraTimer(False)
Call FXTimer(False)
Call HoraTimer(False)

'7. Cerramos los forms y nos vamos
Call UnloadAllForms
End
...
Public Sub AddtoRichTextBox(RichTextBox As RichTextBox, Text As String, Optional red As Integer = -1, Optional green As Integer, Optional blue As Integer, Optional bold As Boolean, Optional italic As Boolean, Optional bCrLf As Boolean, Optional FontTypeIndex As Byte = 0)
    With RichTextBox
...
Public Sub Map_Load(ByVal Map_Num As Integer, ByVal Ambient_Type As Byte, Optional ByVal base_light As Long = 0, Optional ByVal Day_Night_State As Byte = 0)
 
If General_File_Exists(App.Path & "\Mapas\" & "Mapa" & Map_Num & ".iam", vbNormal) Then
                                                                  
    'Si es la vers correcta cambiamos el mapa
    If Engine.Map_Load_From_File(App.Path & "\Mapas\" & "Mapa" & Map_Num & ".iam", True) Then
        ...
        Else
            ...
            If base_light <> 0 Then Engine.Map_Base_Light_Set Val(base_light)
        End If
        
        Engine.Map_Name_Set (UserLocation(Map_Num))
...
Else
    ...
    Call MsgBox("No se ha encontrado el mapa " & Map_Num & " por favor baje nuevamente el juego desde www.imperiumao.com.ar.", vbCritical, "Saliendo")
    Call EndGame
End If
...
If Not CurrentUser.Pausa And frmMain.Visible And Not frmForo.Visible And _
    Not frmComerciar.Visible And Not frmComercioSeguro.Visible And Not frmGoliath.Visible And CurrentUser.Logged And Not frmIniciando.Visible Then
