...
Case 1 '/ban motivo@nick
    ...
    If MsgBox("¿Está seguro que desea banear al personaje """ & cboListaUsus.Text & """?", vbYesNo + vbQuestion) = vbYes Then
        Call ClientTCP.Send_Data_Command_GM(cmdBan, tmp & "@" & cboListaUsus.Text)
    End If
Case 2 '/sum nick
...
Case 27
    Call ClientTCP.Send_Data_Command_GM(cmdApagar)
Case 28
...
If List1.Visible Then
    If List1.ListIndex < 0 Then Exit Sub
    Call ClientTCP.Send_Data(GM_Sos_Manual, Nick)
    List1.RemoveItem List1.ListIndex
    txtMsg.Text = ""
End If
